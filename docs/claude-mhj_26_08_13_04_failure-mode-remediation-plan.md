...
> **2026-08-13 정정.** 아래 원 서술은 두 군데가 틀렸다. `R-7` 착수 시 실측으로
> 확인한 내용을 §3 `R-7`에 반영했다.
>
> - **틀림:** "총 예산이 run 누적이다." → `started = time.monotonic()`이
>   `_run_loop` 안에 있고 그 함수는 프로세스당 한 번 실행되므로, 총 예산은
>   **프로세스당**이다. resume마다 새로 시작한다.
> - **틀림:** "게이트 대기가 총예산을 쓴다." → 게이트가 열리면 `_run_loop`이
>   `return`으로 빠져나가므로 이미 예산을 쓰지 않는다. 옵션 (b)는 이미 참이었다.
> - **놓침:** 실제 해악은 예산 소진이 `FAILED`를 커밋한다는 것이다
>   (`run_loop.py:1604`). 단지 느렸을 뿐인 run이 §11의 재개 금지 대상이 된다.

기본값 `step=900_000`(15분), `total=7_200_000`(2시간)
(`config.py:64-66`). 총 타임아웃 검사가 반복 시작 지점에만 있어(`:1594`) 실제
벽시계 상한은 `total + step`이다.
...
#### `R-7` 예산 소진을 재개 가능한 정지로

`F-5` 해결. 원래 제시한 (a) 기본값 상향 / (b) 게이트 대기 제외 / (c) 스텝 단위
예산은 모두 채택하지 않았다. (b)는 이미 참이었고, (a)와 (c)는 실제 해악을 건드리지
않는다.

채택한 방향: **예산 소진이 `FAILED` 대신 `status=BLOCKED`를 커밋하고 resumable
정지 이벤트를 남긴다.** 예산은 한 코디네이터 프로세스를 제한할 뿐이며, 오래 걸린
run은 깨진 것이 아니라 진행 중인 정상 작업이다. 이 방향은 `R-2`의 분류 체계
(타임아웃은 계약 위반이 아니라 `INTERRUPTED`)와도 일관된다.

부수 개선: 기존 코드는 `active=None`을 커밋해 활성 스텝 바인딩을 버렸다.
`FAILED`일 때는 무해했으나 재개 가능해진 이상 워커 펜싱 정보를 잃으면 안 되므로
`active`를 보존한다.

기본값은 변경하지 않는다. 상한은 여전히 폭주를 막는 역할을 한다.
...
| `R-1`, `R-2`, `R-3`, `R-4` | **완료** — `abf1793`. 설계는 `claude-mhj_26_08_13_05`~`07` |
| `R-7` | **완료** — 예산 소진을 재개 가능한 정지로 전환 |
